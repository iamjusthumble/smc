...
import RejectCompany from "./create";
...
const BusesPage = () => {
  ...
  const { loading, data, refetch } = useQuery(GET_ALL_BUSES, {
    variables: {
      pagination: {
        limit: limit,
        skip: skip,
  ...
  const requests = {
    rows: data?.getAllBuses?.rows,
    count: data?.getAllBuses?.count,
  };
                  ...
                  >
                      ...
                      <span
                        className={classNames(
                          item?.status === "DECOMMISSIONED"
                            ? `bg-yellow-200 text-[#D4A017] dark:bg-[#D4A017] dark:text-gray-50`
                            : "",
                          item?.status === "ACTIVE"
                    ...
                    <td className="whitespace-nowrap px-6 py-4 text-sm text-center text-gray-500 border-b border-gray-200">
                      <div className="space-x-1">
                        ...
                        <ActionButton
                          action="decommission"
                          onClick={dispatchAction(item._id, "decommission")}
                        />
                      </div>
                    </td>
                  </tr>
