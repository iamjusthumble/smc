import React from "react";
import { ITripData } from "./types";
import { FaLocationDot } from "react-icons/fa6";
import { RiRecordCircleFill } from "react-icons/ri";

type ListTripsProps = {
  data: ITripData;
  dispatchAction?: any;
  setSelectedData?: any;
};

function TripBody({ data, dispatchAction }: ListTripsProps) {
  const {
    date: { dayInWords, dayInNumber },
    origin,
    destination,
    numberOfBusAssigned,
    timeScheduled: { startTime, endTime },
    tripType,
  } = data;
  return (
    <>
      <div className="w-full h-40 min-w-[66rem] mx-3 md:mx-0 relative rounded-xl mb-7 shadow-md  border border-slate-200">
        <div className="flex justify-around">
          <div
            style={{ flex: "1" }}
            className="flex flex-col justify-center items-start px-10"
          >
            <h3 className="text-red-500 text-xl pl-1 font-manrope">
              {dayInWords}
            </h3>
            <h1 className="text-red-400 font-medium text-5xl font-manrope">
              {dayInNumber}
            </h1>
          </div>
          <div
            style={{ flex: "4" }}
            className="flex gap-x-10 -ml-28 items-center py-7"
          >
            <div className="h-24 w-[0.7px] bg-gray-500" />
            <div className="flex flex-col items-center">
              <RiRecordCircleFill className="text-gray-500" />
              <div className="h-8 border-r-2 border-dashed border-gray-500 " />
              <FaLocationDot className="text-gray-500" />
            </div>
            <div className="flex flex-col gap-y-5 justify-center">
              <h4 className="text-gray-500 font-medium">{origin.name}</h4>
              <h4 className="text-gray-500 font-medium">{destination.name}</h4>
            </div>
            <div className="h-24 w-[0.7px] bg-gray-500" />

            <div className="flex flex-col gap-y-4 justify-center">
              <h4 className="text-gray-500 font-medium">Total Bus assigned</h4>
              <h4 className="text-gray-500 font-medium">
                {numberOfBusAssigned}
              </h4>
            </div>
            <div className="h-24 w-[0.7px] bg-gray-500" />

            <div className="flex flex-col gap-y-1 justify-center">
              <h4 className="text-gray-500 font-medium">Time Scheduled</h4>
              <h4 className="text-red-500">
                {startTime} - {endTime}
              </h4>
              <h4 className="text-primary">{tripType}</h4>
            </div>
          </div>
          <div style={{ flex: "1" }} className="flex flex-col justify-center">
            <button
              onClick={() => {
                dispatchAction();
              }}
              className="hover:text-white  text-primary hover:bg-primary mx-4 py-2 rounded-lg"
            >
              Edit
            </button>
            <button
              onClick={() => {
                dispatchAction();
              }}
              className="hover:text-white text-primary hover:bg-primary mx-4 py-2 rounded-lg"
            >
              Cancel
            </button>
            <button
              onClick={() => {
                dispatchAction();
              }}
              className="hover:text-white text-primary hover:bg-primary px-1 mx-4 py-2 rounded-lg"
            >
              Delete
            </button>
          </div>
        </div>
      </div>
    </>
  );
}

export default TripBody;
